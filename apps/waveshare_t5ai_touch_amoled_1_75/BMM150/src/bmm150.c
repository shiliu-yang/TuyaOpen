...
// I2C Initialization using dev_config.c (following GPS codebase pattern)
OPERATE_RET bmm150_i2c_init(void) {
    OPERATE_RET ret = dev_i2c_init();
    if (ret != OPRT_OK) {
...
// I2C write register using dev_config.c (following GPS pattern)
OPERATE_RET bmm150_i2c_write_register(uint8_t addr, uint8_t reg, uint8_t value) {
    return dev_i2c_write(addr, reg, value);
}

// I2C read register using dev_config.c (following GPS pattern)
OPERATE_RET bmm150_i2c_read_register(uint8_t addr, uint8_t reg, uint8_t *buffer, uint8_t length) {
    ...
    return dev_i2c_read_nbytes(addr, reg, buffer, length);
}
