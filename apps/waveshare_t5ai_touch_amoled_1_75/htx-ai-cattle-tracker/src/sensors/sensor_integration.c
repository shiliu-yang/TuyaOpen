...
__attribute__((unused)) static void __gps_task(void *param)
{
    ...
    // GPS shares I2C Port 0 with touch display (GPIO 20/21) - software I2C handles conflicts
    PR_INFO("[GPS] Using I2C Port 0 (GPIO %d/%d) - same as working demo...", GPS_I2C_SCL_PIN, GPS_I2C_SDA_PIN);
    PR_INFO("[GPS] I2C addresses: GPS WR=0x%02X, GPS RD=0x%02X", LC76G_ADDRESS, DEVICE_ADDRESS_R);
    op_ret = dev_i2c_init();
    if (op_ret != OPRT_OK) {
        PR_ERR("[GPS] Failed to initialize I2C Port 0 (error: %d)", op_ret);
    ...
    // Initialize GPS module (this will configure GPIO 39 for reset)
    PR_INFO("[GPS] Initializing LC76G module (addresses: WR=0x%02X, RD=0x%02X)...", LC76G_ADDRESS, DEVICE_ADDRESS_R);
    op_ret = lc76g_init(&g_gps_dev, LC76G_ADDRESS, DEVICE_ADDRESS_R);
    if (op_ret != OPRT_OK) {
        PR_ERR("[GPS] Failed to initialize GPS module (error: %d)", op_ret);
        tal_thread_delete(NULL);
        return;
    }
    ...
    while (1) {
        ...
        if (read_ret != OPRT_OK) {
            ...
            tal_system_sleep(2000); // Wait longer on error
            continue;
        }
...
OPERATE_RET sensor_tasks_start(void)
{
    ...
    if (sg_gps_handle == NULL) {
        static THREAD_CFG_T gps_param = {
            .priority = TASK_GPS_PRIORITY - 3,  // Much higher priority than touch display
