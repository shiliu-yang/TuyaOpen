...
OPERATE_RET dev_uart_init(TUYA_UART_NUM_E port, uint32_t baudrate)
{
    ...
    // Configure UART pins for UART2
    if (port == TUYA_UART_NUM_2) {
        tkl_io_pinmux_config(EXAMPLE_UART_TX_PIN, TUYA_UART2_TX);
        tkl_io_pinmux_config(EXAMPLE_UART_RX_PIN, TUYA_UART2_RX);
        PR_NOTICE("UART2 pinmux configured: TX=P%d, RX=P%d", EXAMPLE_UART_TX_PIN, EXAMPLE_UART_RX_PIN);
    }
