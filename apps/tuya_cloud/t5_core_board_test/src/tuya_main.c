...
extern void example_qrcode_string(const char *string,
                                  void (*fputs)(const char *str), int invert);
 ...
 * @return void
 */
void user_log_output_cb(const char *str) { tkl_log_output(str); }
...
void user_upgrade_notify_on(tuya_iot_client_t *client, cJSON *upgrade) {
  ...
  PR_INFO("HTTPS URL: %s",
          cJSON_GetObjectItem(upgrade, "httpsUrl")->valuestring);
}
...
void user_event_handler_on(tuya_iot_client_t *client, tuya_event_msg_t *event) {
  ...
  case TUYA_EVENT_DP_RECEIVE_OBJ: {
    ...
    PR_DEBUG("SOC Rev DP Cmd t1:%d t2:%d CNT:%u", dpobj->cmd_tp, dpobj->dtt_tp,
             dpobj->dpscnt);
    if (dpobj->devid != NULL) {
    ...
    for (index = 0; index < dpobj->dpscnt; index++) {
      ...
      PR_DEBUG("idx:%d dpid:%d type:%d ts:%u", index, dp->id, dp->type,
               dp->time_stamp);
      switch (dp->type) {
      case PROP_BOOL: {
      ...
      default: {
        PR_ERR("idx:%d dpid:%d type:%d ts:%u is invalid", index, dp->id,
               dp->type, dp->time_stamp);
        break;
      }
      } // end of switch
    }
...
void user_main() {
  ...
  if (OPRT_OK != tuya_authorize_read(&license)) {
    ...
    PR_WARN(
        "Replace the TUYA_OPENSDK_UUID and TUYA_OPENSDK_AUTHKEY contents, otherwise the demo cannot work.\n \
...
#if defined(ENABLE_WIFI) && (ENABLE_WIFI == 1)
  netmgr_conn_set(
      NETCONN_WIFI, NETCONN_CMD_NETCFG,
  ...
  for (;;) {
    tal_system_sleep(1000); // sleep 1s
  }
#endif
}
