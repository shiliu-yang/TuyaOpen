...
OPERATE_RET tuya_ai_input_cloud_trigger(const char *request_id, const char *content)
{
    ...
    // trigger->request_id = strdup(request_id);
    // trigger->content = strdup(content);
...
void __ai_mq_do_text(cJSON *root)
{
    // {"data":
    // {"requestId":"trigger_f17fxcv84d8g","content":"请使用简洁清晰的表达方式提醒用户有牛移动到了村里的菜地附近，可能有偷吃菜的风险，请用户有时间可以去菜地看一。"},"action":"TriggerAichat"},"bizId":"Trigger_f17fxcv84d8h"}
