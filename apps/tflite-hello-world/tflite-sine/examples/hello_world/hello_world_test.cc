...
#include "models/sine_model_quantized_model_data.h"
...
using HelloWorldQuantizedOpResolver = tflite::MicroMutableOpResolver<3>;
...
TfLiteStatus RegisterQuantizedOps(HelloWorldQuantizedOpResolver& op_resolver) {
  TF_LITE_ENSURE_STATUS(op_resolver.AddFullyConnected());
...
TfLiteStatus LoadQuantizedSineModelAndPerformInference() {
  // Map the model into a usable data structure. This doesn't involve any
  // copying or parsing, it's a very lightweight operation.
  const tflite::Model* model =
      ::tflite::GetModel(g_sine_model_quantized_model_data);
  TFLITE_CHECK_EQ(model->version(), TFLITE_SCHEMA_VERSION);

  HelloWorldQuantizedOpResolver op_resolver;
  TF_LITE_ENSURE_STATUS(RegisterQuantizedOps(op_resolver));
  ...
  for (int i = 0; i < kNumTestValues; ++i) {
    ...
    TF_LITE_ENSURE_STATUS(interpreter.Invoke());
    float y_pred = (output->data.int8[0] - output_zero_point) * output_scale;
    float expected = sin(golden_inputs_float[i]);
    float diff = fabs(expected - y_pred);
        // Explicitly cast to double to silence -Wdouble-promotion under -Werror.
        MicroPrintf("[QUANTIZED SINE] x=%f expected=%f pred=%f diff=%f epsilon=%f\n",
          static_cast<double>(golden_inputs_float[i]),
...
int tflite_main(int argc, char* argv[]) {
  ...
  TF_LITE_ENSURE_STATUS(LoadQuantizedSineModelAndPerformInference());
  MicroPrintf("~~~ALL TESTS PASSED~~~\n");
  return kTfLiteOk;
}

extern "C" int tflite_main_c_function(int argc, char* argv[]) {
  return tflite_main(argc, argv);
}
