...
// for Waveshare-ESP32-S3-Touch-AMOLED-1.8
...
typedef struct {
    ...
    esp_timer_handle_t notification_tm_id;
} APP_CHATBOT_UI_T;
...
static void __ui_font_init(APP_UI_FONT_T *font)
{
    ...
    // for Waveshare-ESP32-S3-Touch-AMOLED-1.8
    font->text = &font_puhui_30_4;
    font->icon = &font_awesome_30_4;

    extern const lv_font_t *font_emoji_64_init(void);
    font->emoji = font_emoji_64_init();

    if (NULL == font->emoji) {
        ESP_LOGE(TAG, "font_emoji_64_init failed");
    }
}

static void __ui_notification_timeout_cb(void *arg)
{
    lvgl_port_lock(0);
...
void ui_frame_init(void)
{
    ...
    // For Waveshare-ESP32-S3-Touch-AMOLED-1.8
    lv_obj_set_style_pad_left(sg_ui.ui.status_bar, LV_HOR_RES * 0.1, 0);
    lv_obj_set_style_pad_right(sg_ui.ui.status_bar, LV_HOR_RES * 0.1, 0);
    ...
    esp_timer_create_args_t timer_args = {
        .callback = &__ui_notification_timeout_cb,
    };
    esp_timer_create(&timer_args, &sg_ui.notification_tm_id);
...
void ui_set_notification(const char *notification)
{
    ...
    esp_timer_start_once(sg_ui.notification_tm_id, 3 * 1000 * 1000);
