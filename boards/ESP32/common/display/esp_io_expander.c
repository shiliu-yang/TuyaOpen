                                  ...
                                  esp_io_expander_dir_t direction)
{
    ...
    temp = dir_reg;
    if ((is_output && !handle->config.flags.dir_out_bit_zero) ||
...
esp_err_t esp_io_expander_set_level(esp_io_expander_handle_t handle, uint32_t pin_num_mask, uint8_t level)
{
    ...
    for (int i = 0; i < io_count; i++) {
        if (pin_num_mask & BIT(i)) {
            ...
            if ((dir_bit && handle->config.flags.dir_out_bit_zero) ||
    ...
    if ((level && !handle->config.flags.output_high_bit_zero) ||
        (!level && handle->config.flags.output_high_bit_zero)) {
...
static esp_err_t write_reg(esp_io_expander_handle_t handle, reg_type_t reg, uint32_t value)
{
    ...
    case REG_DIRECTION:
        ESP_RETURN_ON_FALSE(handle->write_direction_reg, ESP_ERR_NOT_SUPPORTED, TAG,
                            "write_direction_reg isn't implemented");
...
static esp_err_t read_reg(esp_io_expander_handle_t handle, reg_type_t reg, uint32_t *value)
{
    ...
    case REG_DIRECTION:
        ESP_RETURN_ON_FALSE(handle->read_direction_reg, ESP_ERR_NOT_SUPPORTED, TAG,
                            "read_direction_reg isn't implemented");
