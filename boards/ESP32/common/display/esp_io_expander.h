...
typedef struct {
    ...
    struct {
        uint8_t dir_out_bit_zero : 1;     /*!< If the direction of IO is output, the corresponding bit of the direction
                                             register is 0 */
        uint8_t input_high_bit_zero : 1;  /*!< If the input level of IO is high, the corresponding bit of the input
                                             register is 0 */
        uint8_t output_high_bit_zero : 1; /*!< If the output level of IO is high, the corresponding bit of the output
                                             register is 0 */
    } flags;
...
struct esp_io_expander_s {
     ...
     * @note If there are multiple input registers in the device, their values should be spliced together in order to
     * form the `value`.
     *
     ...
     * @note This function can be implemented by reading the physical output register, or simply by reading a variable
     * that record the output value (more faster)
     * @note If there are multiple input registers in the device, their values should be spliced together in order to
     * form the `value`.
     *
     * @param handle: IO Expander handle
     ...
     * @note This function can be implemented by reading the physical direction register, or simply by reading a
     * variable that record the direction value (more faster)
     * @note If there are multiple input registers in the device, their values should be spliced together in order to
     * form the `value`.
     *
     * @param handle: IO Expander handle
 ...
 *
 * @note All target IOs must be in output mode first, otherwise this function will return the error
