/**
 * @file at_line.c
 * @brief at_line module is used to
 * @version 0.1
 * @copyright Copyright (c) 2021-2025 Tuya Inc. All Rights Reserved.
 */

#include "at_line.h"

#include "tal_api.h"

/***********************************************************
************************macro define************************
***********************************************************/
#define AT_LINE_MALLOC tal_psram_malloc
#define AT_LINE_FREE   tal_psram_free

#define AT_LINE_MAGIC 0x12345678

/***********************************************************
***********************typedef define***********************
***********************************************************/
typedef struct {
    uint32_t magic;
    SLIST_HEAD line_head;
    uint32_t line_count;
    MUTEX_HANDLE mutex;
} AT_LINE_HANDLE_T;

/***********************************************************
********************function declaration********************
***********************************************************/

/***********************************************************
***********************variable define**********************
***********************************************************/

/***********************************************************
***********************function define**********************
***********************************************************/
OPERATE_RET at_line_init(AT_LINE_HANDLE *line_hdl)
{
    OPERATE_RET rt = OPRT_OK;
    AT_LINE_HANDLE_T *line = NULL;

    TUYA_CHECK_NULL_RETURN(line_hdl, OPRT_INVALID_PARM);

    line = AT_LINE_MALLOC(sizeof(AT_LINE_HANDLE_T));
    // TUYA_CHECK_NULL_RETURN(line, OPRT_MALLOC_FAILED);
    if (line == NULL) {
        PR_ERR("malloc for AT_LINE_HANDLE_T failed");
        rt = OPRT_MALLOC_FAILED;
        goto __EXIT;
    }
    PR_DEBUG("malloc for AT_LINE_HANDLE_T success: %p", line);
    // memset(line, 0, sizeof(AT_LINE_HANDLE_T));
    line->magic = AT_LINE_MAGIC;
    line->line_count = 0;
    line->mutex = NULL;
    tuya_init_slist_node(&line->line_head);

    TUYA_CALL_ERR_GOTO(tal_mutex_create_init(&line->mutex), __EXIT);

    // tal_mutex_lock(line->mutex);

    // line->magic = AT_LINE_MAGIC;

    // line->line_count = 0;

    *line_hdl = (AT_LINE_HANDLE)line;
    // tal_mutex_unlock(line->mutex);

__EXIT:
    if (OPRT_OK != rt) {
        at_line_deinit((AT_LINE_HANDLE)line);
        line = NULL;
    }

    return rt;
}

AT_LINE_T *at_line_create(const char *line, uint32_t len)
{
    TUYA_CHECK_NULL_RETURN(line, NULL);

    AT_LINE_T *new_line = (AT_LINE_T *)AT_LINE_MALLOC(sizeof(AT_LINE_T) + len + 1);
    TUYA_CHECK_NULL_RETURN(new_line, NULL);
    memset(new_line, 0, sizeof(AT_LINE_T));

    new_line->extra = NULL;
    new_line->len = len;
    new_line->data = (char *)(new_line + 1); // Point to the memory after the struct

    memcpy(new_line->data, line, len);
    new_line->data[len] = '\0'; // Null-terminate the string

    return new_line;
}

OPERATE_RET at_line_add(AT_LINE_HANDLE line_hdl, AT_LINE_T *new_line)
{
    OPERATE_RET rt = OPRT_OK;

    TUYA_CHECK_NULL_RETURN(line_hdl, OPRT_INVALID_PARM);
    TUYA_CHECK_NULL_RETURN(new_line, OPRT_INVALID_PARM);

    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;
    if (at_line->magic != AT_LINE_MAGIC) {
        PR_ERR("Invalid AT line handle");
        return OPRT_INVALID_PARM;
    }

    tal_mutex_lock(at_line->mutex);
    tuya_init_slist_node(&new_line->node);
    tuya_slist_add_tail(&at_line->line_head, &new_line->node);
    at_line->line_count++;
    tal_mutex_unlock(at_line->mutex);

    return rt;
}

AT_LINE_T *at_line_get(AT_LINE_HANDLE line_hdl)
{
    AT_LINE_T *line = NULL;

    TUYA_CHECK_NULL_RETURN(line_hdl, NULL);
    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;
    if (at_line->magic != AT_LINE_MAGIC) {
        PR_ERR("Invalid AT line handle");
        return NULL;
    }

    tal_mutex_lock(at_line->mutex);

    if (0 == at_line->line_count) {
        // PR_DEBUG("No lines available");
        goto __EXIT;
    }

    line = (AT_LINE_T *)at_line->line_head.next;

    // Remove the line from the list
    tuya_slist_del(&at_line->line_head, &line->node);
    at_line->line_count--;

__EXIT:
    tal_mutex_unlock(at_line->mutex);

    return line;
}

AT_LINE_T *at_line_get_with_prefix(AT_LINE_HANDLE line_hdl, const char *prefix)
{
    // OPERATE_RET rt = OPRT_OK;
    AT_LINE_T *found_line = NULL;

    TUYA_CHECK_NULL_RETURN(line_hdl, NULL);
    TUYA_CHECK_NULL_RETURN(prefix, NULL);

    if (strlen(prefix) == 0) {
        PR_ERR("Prefix is empty");
        return NULL;
    }

    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;
    if (at_line->magic != AT_LINE_MAGIC) {
        PR_ERR("Invalid AT line handle");
        return NULL;
    }

    tal_mutex_lock(at_line->mutex);

    if (0 == at_line->line_count) {
        // PR_DEBUG("No lines available");
        goto __EXIT;
    }

    AT_LINE_T *current_line = NULL;
    SLIST_HEAD *node = NULL;

    node = at_line->line_head.next;
    while (node != &at_line->line_head) {
        current_line = (AT_LINE_T *)node;
        if (strncmp(current_line->data, prefix, strlen(prefix)) == 0) {
            found_line = current_line;
            break;
        }
        node = node->next;
    }

    if (found_line) {
        // Remove the found line from the list
        tuya_slist_del(&at_line->line_head, &found_line->node);
        at_line->line_count--;
    }

__EXIT:
    tal_mutex_unlock(at_line->mutex);

    return found_line;
}

OPERATE_RET at_line_set_extra(AT_LINE_HANDLE line_hdl, AT_LINE_T *line, void *extra)
{
    OPERATE_RET rt = OPRT_OK;

    TUYA_CHECK_NULL_RETURN(line, OPRT_INVALID_PARM);
    TUYA_CHECK_NULL_RETURN(line_hdl, OPRT_INVALID_PARM);

    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;
    if (at_line->magic != AT_LINE_MAGIC) {
        PR_ERR("Invalid AT line handle");
        return OPRT_INVALID_PARM;
    }

    tal_mutex_lock(at_line->mutex);
    line->extra = extra;
    tal_mutex_unlock(at_line->mutex);

    return rt;
}

void *at_line_first_get_extra(AT_LINE_HANDLE line_hdl)
{
    void *rt_extra = NULL;

    TUYA_CHECK_NULL_RETURN(line_hdl, NULL);
    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;

    if (at_line->magic != AT_LINE_MAGIC) {
        PR_ERR("Invalid AT line handle");
        return NULL;
    }

    tal_mutex_lock(at_line->mutex);

    if (0 == at_line->line_count) {
        // PR_DEBUG("No lines available");
        goto __EXIT;
    }

    AT_LINE_T *line = (AT_LINE_T *)at_line->line_head.next;
    if (line) {
        rt_extra = line->extra;
    }

__EXIT:
    tal_mutex_unlock(at_line->mutex);

    return rt_extra;
}

OPERATE_RET at_line_free(AT_LINE_T *line)
{
    OPERATE_RET rt = OPRT_OK;

    TUYA_CHECK_NULL_RETURN(line, OPRT_INVALID_PARM);

    AT_LINE_FREE(line);

    return rt;
}

OPERATE_RET at_line_clean(AT_LINE_HANDLE line_hdl)
{
    OPERATE_RET rt = OPRT_OK;
    AT_LINE_T *line = NULL;

    TUYA_CHECK_NULL_RETURN(line_hdl, OPRT_INVALID_PARM);
    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;

    tal_mutex_lock(at_line->mutex);

    if (at_line->line_count == 0) {
        goto __EXIT;
    }

    while (1) {
        if (tuya_slist_empty(&(at_line->line_head))) {
            break;
        }
        line = (AT_LINE_T *)at_line->line_head.next;

        // Remove the line from the list
        tuya_slist_del(&(at_line->line_head), &line->node);
        at_line->line_count--;

        at_line_free(line);
        line = NULL;
    }

    at_line->line_count = 0;

    PR_DEBUG("at_line_clean");

__EXIT:
    tal_mutex_unlock(at_line->mutex);

    return rt;
}

uint32_t at_line_get_count(AT_LINE_HANDLE line_hdl)
{
    uint32_t count = 0;

    if (line_hdl == NULL) {
        return 0;
    }

    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;
    if (at_line->magic != AT_LINE_MAGIC) {
        PR_ERR("Invalid AT line handle");
        return 0;
    }

    tal_mutex_lock(at_line->mutex);
    count = at_line->line_count;
    tal_mutex_unlock(at_line->mutex);

    return count;
}

OPERATE_RET at_line_deinit(AT_LINE_HANDLE line_hdl)
{
    OPERATE_RET rt = OPRT_OK;

    TUYA_CHECK_NULL_RETURN(line_hdl, OPRT_INVALID_PARM);

    AT_LINE_HANDLE_T *at_line = (AT_LINE_HANDLE_T *)line_hdl;
    if (at_line->magic != AT_LINE_MAGIC) {
        PR_ERR("Invalid AT line handle");
        return OPRT_INVALID_PARM;
    }

    TUYA_CALL_ERR_LOG(at_line_clean(line_hdl));

    // Release mutex
    if (at_line->mutex) {
        tal_mutex_release(at_line->mutex);
        at_line->mutex = NULL;
    }

    // Free the handle itself
    AT_LINE_FREE(at_line);

    return rt;
}
